...
// Default to PFFFT if no backend is explicitly selected (useful for non-CMake builds like Bela)
#if !defined(USE_FFTW) && !defined(USE_KISS_FFT) && !defined(USE_ACCELERATE_FFT) && !defined(USE_PFFFT)
#define USE_PFFFT
#endif

// Enforce that exactly one backend is selected
#if ((defined(USE_FFTW) + defined(USE_KISS_FFT) + defined(USE_ACCELERATE_FFT) + defined(USE_PFFFT)) != 1)
#error "Define exactly one FFT backend: USE_FFTW, USE_KISS_FFT, USE_ACCELERATE_FFT, or USE_PFFFT."
#endif
...
public:
...
#ifdef USE_BITSTREAM_PITCH
    // Optional tuning for bitstream pitch prototype
    void setBitstreamPitchRange(float lowestHz, float highestHz);
    void setBitstreamPitchHysteresis(float hysteresisLinear);
#endif
    ...
    /** Process an already windowed audio frame without copying it into internal storage.
     * This avoids an extra copy and is suitable for real-time audio threads.
     * Note: time-domain feature queries (e.g., RMS) will reflect the last call to
     * processAudioFrame() that updated internal storage, not this fast path.
     * @param frame a pointer to a windowed audio frame (length == frameSize)
     * @param numSamples number of samples in the frame
     */
    void processWindowedFrame (const T* frame, int numSamples);
    ...
    /** @Returns a fast monophonic pitch estimate using a bitstream
     *  autocorrelation prototype. If not compiled with USE_BITSTREAM_PITCH,
     *  this falls back to pitch(). */
    T pitchFast();
...
#ifdef USE_PFFFT
    PFFFT_Setup* pffftSetup;           /**< PFFFT setup (float only) */
    float* pffftInput;                 /**< aligned input buffer (float) */
    float* pffftOutput;                /**< aligned output buffer (float) */
    float* pffftWork;                  /**< aligned scratch (float) */
#endif
...
#ifdef USE_BITSTREAM_PITCH
    /** bitstream autocorrelation pitch (prototype) */
    BitstreamPitch bitstreamPitch;
#endif
};
